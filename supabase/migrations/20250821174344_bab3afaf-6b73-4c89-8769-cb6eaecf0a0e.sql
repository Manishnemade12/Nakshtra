...
CREATE POLICY "Users can view their own study plans" ON public.study_plans FOR SELECT USING (user_id = auth.uid());
CREATE POLICY "Users can create their own study plans" ON public.study_plans FOR INSERT WITH CHECK (user_id = auth.uid());
CREATE POLICY "Users can update their own study plans" ON public.study_plans FOR UPDATE USING (user_id = auth.uid());
CREATE POLICY "Users can delete their own study plans" ON public.study_plans FOR DELETE USING (user_id = auth.uid());
