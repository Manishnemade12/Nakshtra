  ...
  return (
    <div className="flex flex-col h-screen max-w-md mx-auto border border-gray-200 rounded-2xl shadow-lg bg-gray-50">
      ...
      <main className="flex-1 overflow-y-auto p-4 bg-gray-100">
        {messages.length === 0 && (
          <p className="text-center text-gray-500 text-base opacity-80 select-none">
            Say hello! Type or use voice input.
          </p>
        )}
        ...
        {messages.map((msg, idx) => (
          ...
          >
            {msg.role === "assistant" && (
              <div className="relative bg-white border border-gray-200 rounded-xl shadow-md px-4 py-3 max-w-[75%] text-gray-700 text-sm leading-relaxed">
                <ReactMarkdown>{msg.content}</ReactMarkdown>
                <button
                  onClick={() => (isSpeaking ? stopSpeaking() : speakText(msg.content))}
                  className="absolute bottom-1 right-1 text-blue-500 text-xs flex items-center gap-1"
                >
                  {isSpeaking ? <VolumeX size={14} /> : <Volume2 size={14} />}
            ...
            {msg.role === "user" && (
              <div className="bg-blue-500 text-white rounded-xl shadow-md px-4 py-3 max-w-[75%] text-sm leading-relaxed">
                {msg.content}
              </div>
            )}
          </div>
      ...
      >
        <button
          ...
          className={`flex items-center justify-center h-10 w-10 rounded-full transition ${
            isRecording
              ? "bg-blue-100 text-cyan-600 cursor-not-allowed"
        ...
        <input
          ...
          disabled={isTyping || isRecording}
          placeholder={isRecording ? "Listening..." : "Type message..."}
          className="flex-1 rounded-full border border-gray-300 px-4 py-2 text-sm text-gray-700 focus:outline-none focus:ring-2 focus:ring-blue-400 disabled:bg-gray-100"
        />
        ...
        <button
          ...
          className={`h-10 w-10 rounded-full flex items-center justify-center transition ${
            isTyping || !input.trim() || isRecording
